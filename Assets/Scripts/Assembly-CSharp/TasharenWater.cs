...
public class TasharenWater : MonoBehaviour
{
    ...
    private Camera GetReflectionCamera(Camera current, Material mat, int textureSize)
    {
        if (!mTex || mTexSize != textureSize)
        {
            ...
            mTex.name = "__MirrorReflection" + GetEntityId();
        ...
        if (!camera)
        {
            GameObject gameObject = new GameObject("Mirror Refl Camera id" + GetEntityId() + " for " + current.GetEntityId(), typeof(Camera), typeof(Skybox));
