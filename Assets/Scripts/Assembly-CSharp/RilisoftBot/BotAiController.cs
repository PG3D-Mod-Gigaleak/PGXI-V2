...
namespace RilisoftBot
{
    public class BotAiController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            InitTeleportData();
            NavMeshAgentSafe.EnsureOnNavMesh(_naveMeshAgent);
        }
        ...
        private void UpdatePatrolState()
        {
            if (IsCanMove && _lastTimeMoving <= Time.time)
            {
                ...
                if (!NavMeshAgentSafe.SetDestination(_naveMeshAgent, _targetPoint))
                {
                    _lastTimeMoving = 0f;
                    return;
                }
        ...
        private void UpdateMoveToTargetState()
        {
            ...
            if (!NavMeshAgentSafe.EnsureOnNavMesh(_naveMeshAgent))
            {
                return;
            }
            ...
            if (!NavMeshAgentSafe.SetDestination(_naveMeshAgent, currentTarget.position))
            {
                return;
            }
            if (_typeBot == TypeBot.Melee)
        ...
        {
            NavMeshAgentSafe.ResetPath(_naveMeshAgent);
        }
